...
export type Verdict = {
  ...
  mode?: "walk-back" | "floor-repair" | "cap-skip" | "rebalance" | "at-target";
  flags?: string[];
};
...
export function isFomcPassed(now = new Date()): boolean {
  return now.getTime() >= new Date(FOMC_AT).getTime();
}

export function walkBackActive(pmi: number, curveInverted: boolean): boolean {
  return pmi < 50 && curveInverted;
}

export function idtlFlag(gt30: number): boolean {
...
function renormalize(parts: { ticker: string; raw: number }[], contribution: number): Allocation[] {
  const total = parts.reduce((s, p) => s + p.raw, 0);
  if (total <= 0) return [];
  const allocs = parts.map((p) => {
    const pct = (p.raw / total) * 100;
    return { ticker: p.ticker, pct, pounds: (pct / 100) * contribution };
  });
  const pctSum = allocs.reduce((s, a) => s + a.pct, 0);
  if (allocs.length && Math.abs(pctSum - 100) > 0.05) {
  ...
  return allocs.map((a) => ({ ...a, pct: roundPct(a.pct), pounds: Math.round(a.pounds) }));
}

export function currentWeight(weights: Record<string, number>, line: Line): number {
  return weights[line.ticker] ?? line.weight;
}

export function routeContribution(pie: Line[], state: MachineState): Verdict {
  const { pmi, curveInverted, contribution, weights } = state;
  const flags: string[] = [];

  if (walkBackActive(pmi, curveInverted)) {
    return {
      kind: "walk-back",
      stamp: "WALK-BACK",
      rule: "PMI < 50 AND 2s10s inverted → all contributions route to XSTR.",
      reason:
        `PMI is ${pmi.toFixed(1)} and the 2s10s curve is inverted. Satellites receive zero. The reserve builds. When the cycle turns, deploy into VWRP, ICOM and SGLN at depressed prices.`,
      ifThen: IF_THEN,
      mode: "walk-back",
      allocations: [{ ticker: "XSTR", pct: 100, pounds: contribution }],
  ...
  if (vwrp < 40) {
    return {
      ...
      ifThen: IF_THEN,
      mode: "floor-repair",
      allocations: [{ ticker: "VWRP", pct: 100, pounds: contribution }],
    };
  }

  if (vwrp > 50) {
    flags.push("VWRP is above the 50% cap — it receives nothing this month.");
  }

  const belowFloor = pie.filter((l) => {
    const w = currentWeight(weights, l);
    if (l.ticker === "VWRP" && vwrp > 50) return false;
    return w < l.floor - 0.05;
  });
  ...
  const eligible = pie.filter((l) => {
    const w = currentWeight(weights, l);
    if (l.ticker === "VWRP" && vwrp >= 50) return false;
    if (w >= l.cap - 0.05) return false;
    if (l.cap <= 5 && l.ticker !== "VWRP" && l.ticker !== "ICOM" && l.ticker !== "SGLN" && l.ticker !== "ITPS" && l.ticker !== "VAGS" && l.ticker !== "XSTR" && w >= 5) {
      return false;
    }
    return true;
  });

  const underweight = eligible.filter((l) => currentWeight(weights, l) < l.weight - 0.05);

  if (underweight.length === 0) {
    const atTarget = pie.filter((l) => {
      const w = currentWeight(weights, l);
      if (l.ticker === "VWRP" && vwrp >= 50) return false;
      if (w >= l.cap - 0.05) return false;
      return true;
    });
    const parts = atTarget.map((l) => ({ ticker: l.ticker, raw: l.weight }));
    return {
      kind: "route",
      stamp: "AT TARGET",
      rule: "Weights inside band. Contribute at target. The 3rd does the work.",
      reason:
        "No line is below its floor. VWRP is inside 40–50. Route this month’s contribution at target weights. Do not sell to rebalance.",
      ifThen: IF_THEN,
      mode: "at-target",
      allocations: renormalize(parts, contribution),
      flags,
  ...
  return {
    ...
    reason: `Underweight: ${underweight
      .map((l) => `${l.ticker} ${currentWeight(weights, l).toFixed(1)}% vs ${l.weight}%`)
      .join("; ")}. Overweight lines receive nothing. Nothing is sold.`,
    ifThen: IF_THEN,
    mode: "rebalance",
    allocations: renormalize(parts, contribution),
    flags,
...
export function verdictSell(pie: Line[], ticker: string): Verdict {
  ...
  const partners =
    line.partnersWhenSuffers.length > 0
      ? `When it suffers, ${line.partnersWhenSuffers.join(", ")} earn.`
      : "This line does not suffer in isolation — it is the reserve.";
  return {
    kind: "blocked",
    stamp: "DO NOT SELL",
    ...
    ifThen: IF_THEN,
    flags: line.probation ? [line.probation] : undefined,
  };
}

export function verdictAddLine(ticker: string): Verdict {
  const dead = PERMANENTLY_DEAD.find(
    (d) =>
      d.name.toLowerCase().includes(ticker.toLowerCase()) ||
      d.id === ticker.toLowerCase(),
  );
  if (dead) {
    return {
      kind: "dead",
...
export function verdictAutoInvest(state: MachineState, now = new Date()): Verdict {
  ...
  if (!state.signedAt) {
    if (!fomc) {
      return {
        ...
        reason:
          "The 16 September 2026 FOMC has not been signed. Next action: sign the sentence. Write the four lock-file entries. Then change AutoInvest on 17 September. Not before. A briefing is not a change log entry.",
    ...
    return {
      ...
      reason:
        "FOMC has occurred. The sentence is not yet signed. Sign first. Then AutoInvest. Same-day change is still a protocol break — decision date precedes change date.",
      ifThen: IF_THEN,
    };
  }
  if (state.pieMode !== "revised") {
    return {
      kind: "blocked",
      stamp: "PIE NOT SWITCHED",
      rule: "The signed sentence names the revised pie.",
      reason:
        "You have a signature, but Command is still on the 12 September lock. Switch the working pie to Revised, then apply the four AutoInvest changes the next calendar day.",
      ifThen: IF_THEN,
    };
  }
  return {
    ...
    reason:
      "Signed. Working pie is Revised. Apply the four entries on the next calendar day — not today. XDWH → XAIX 2%. URNG → NUCL 2.5%. SMGB 2.5% → 1.0%. INXG → TIP5 2.0%. Nothing else.",
    ifThen: "If the four lock-file entries are signed after the 16 September 2026 FOMC, then AutoInvest changes on 17 September and not before.",
    flags: LOCK_ENTRIES.map((e) => `${e.n}. ${e.from} → ${e.to}. ${e.reason}`),
  };
}

export function verdictSizeGate(id: (typeof SIZE_GATES)[number]["id"], nav: number): Verdict {
...
export function verdictDead(id: string): Verdict {
...
export function earners(pie: Line[], regime: Regime): Line[] {
  return pie.filter((l) => l.regimes[regime] === "earn");
}

export function sufferers(pie: Line[], regime: Regime): Line[] {
  return pie.filter((l) => l.regimes[regime] === "suffer");
}

export function fvAnnuityMonthly(monthly: number, annualRate: number, years: number): number {
...
export function pathSeries(monthly: number, annualRate: number, years: number): number[] {
  const r = annualRate / 12;
  const out: number[] = [];
  let v = 0;
